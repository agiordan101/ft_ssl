...
{
    SHA256_CTX ctx;
    // BYTE text1[] = {"42 is nice"};
    BYTE buf[SHA256_BLOCK_SIZE];
    
	sha256_init(&ctx);
	sha256_update(&ctx, av[1], strlen(av[1]));
	sha256_final(&ctx, buf);

    printf("data (len=%d): %s\n", ctx.datalen, ctx.data);
    printBits(ctx.data, 64);
    ...
    for (int i = 0; i < 32; i++)
    {
        if (i && i % 8 == 0)
            printf(" ");
        printf("%x", buf[i]);
        // ft_printHex((unsigned int)buf[i]);
    }
    printf("\n");
}
